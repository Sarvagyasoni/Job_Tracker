...
export function useResume() {
  ...
  const tailorBullets = useCallback(async (bulletPoints: string[], jobDescription: string) => {
    setError(null);
    setIsLoading(true);
    try {
      const response = await resumeApi.tailorBullets(bulletPoints, jobDescription);
      return { success: true, data: response.data };
    } catch (err) {
      const apiError = err as ApiError;
