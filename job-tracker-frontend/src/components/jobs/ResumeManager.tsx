...
import type { ATSScoreResponse, TailoredBullet } from '../../types';
...
export function ResumeManager() {
  ...
  const [tailorBulletsInput, setTailorBulletsInput] = useState('');
  ...
  const bulletCount = tailorBulletsInput.split('\n').map(b => b.trim()).filter(b => b).length;
  const jdCharCount = tailorJobDescription.length;
  const bulletsCharCount = tailorBulletsInput.length;
  const canTailor = bulletCount > 0 && tailorJobDescription.trim().length > 0;
  ...
  const handleTailorBullets = useCallback(async () => {
    ...
    const bullets = tailorBulletsInput.split('\n').map(b => b.trim()).filter(b => b);
    if (bullets.length === 0) {
      toast({ type: 'error', title: 'Missing bullets', message: 'Please enter at least one bullet point.' });
      return;
    }
    setIsTailoring(true);
    setTailoringStage('Analyzing job description...');
    try {
      const result = await tailorBullets(bullets, tailorJobDescription);
      if (result.success && result.data) {
        // Initialize all bullets as pending
        const bulletsWithStatus = result.data.results.map(b => ({ ...b, status: 'pending' as const }));
        setTailoredBullets(bulletsWithStatus);
        setTailoringStage('');
        toast({ type: 'success', title: 'Bullets tailored', message: 'Review and accept changes below.' });
      } else {
        setTailoringStage('');
  ...
  }, [tailorJobDescription, tailorBulletsInput, tailorBullets, toast]);
  ...
  const acceptBullet = useCallback((index: number) => {
    setTailoredBullets(prev => prev?.map((b, i) => i === index ? { ...b, status: 'accepted' as const } : b) || null);
  }, []);

  const rejectBullet = useCallback((index: number) => {
    setTailoredBullets(prev => prev?.map((b, i) => i === index ? { ...b, status: 'rejected' as const } : b) || null);
  }, []);

  const regenerateBullet = useCallback(async (index: number) => {
    if (!tailoredBullets || !tailorJobDescription.trim()) return;
    const originalBullets = tailorBulletsInput.split('\n').map(b => b.trim()).filter(b => b);
    setTailoredBullets(prev => prev?.map((b, i) => i === index ? { ...b, status: 'pending' as const } : b) || null);
    setTailoringStage(`Regenerating bullet ${index + 1}...`);
    try {
      const result = await tailorBullets([originalBullets[index]], tailorJobDescription);
      if (result.success && result.data) {
        setTailoredBullets(prev => prev?.map((b, i) => i === index ? { ...result.data.results[0], status: 'pending' as const } : b) || null);
        toast({ type: 'success', title: 'Regenerated', message: 'New version ready for review.' });
      } else {
        toast({ type: 'error', title: 'Regeneration failed', message: result.error });
      }
    } catch {
  ...
  }, [tailoredBullets, tailorBulletsInput, tailorJobDescription, tailorBullets, toast]);

  const copyAllTailored = useCallback(() => {
    if (!tailoredBullets) return;
    const accepted = tailoredBullets.filter(b => b.status === 'accepted');
    const text = accepted.map(b => b.tailored).join('\n');
    navigator.clipboard.writeText(text);
    toast({ type: 'success', title: 'Copied', message: `${accepted.length} bullet(s) copied to clipboard.` });
  }, [tailoredBullets, toast]);

  const exportTailored = useCallback(() => {
    if (!tailoredBullets) return;
    const accepted = tailoredBullets.filter(b => b.status === 'accepted');
    const text = accepted.map(b => `• ${b.tailored}`).join('\n\n');
    ...
    toast({ type: 'success', title: 'Exported', message: 'Downloaded as tailored-resume-bullets.txt' });
  }, [tailoredBullets, toast]);

  const getOverallScore = () => {
    if (!tailoredBullets) return null;
    const accepted = tailoredBullets.filter(b => b.status === 'accepted');
    if (accepted.length === 0) return null;
    // Calculate average match based on keywords added
    return Math.round(accepted.reduce((sum, b) => sum + (b.keywords_added?.length || 0), 0) / accepted.length * 20);
  };
    ...
    <div className={styles.container}>
      ...
      {activeTab === 'tailor' && (
        <div className={styles.tabContent}>
          {!resume && (
            <div className={styles.emptyState}>
              <p>Upload a resume first, or you can still tailor bullets without a saved resume.</p>
            </div>
          )}
          <div className={styles.inputGrid}>
            <div className={styles.section}>
              <div className={styles.sectionHeader}>
                <label htmlFor="tailor-bullets">Your Bullet Points (one per line)</label>
                <span className={styles.charCount}>{bulletsCharCount} chars • {bulletCount} bullet{bulletCount !== 1 ? 's' : ''}</span>
              </div>
              <textarea
                id="tailor-bullets"
                value={tailorBulletsInput}
                onChange={e => setTailorBulletsInput(e.target.value)}
...
• Implemented CI/CD pipelines reducing deployment time from 30min to 5min"
                rows={6}
                className={styles.textarea}
              />
            </div>
            ...
            >
              {isTailoring ? (
                <>
                  ...
                  {tailoringStage || 'Tailoring...'}
                </>
              ) : (
                'Tailor Bullets'
              )}
            </Button>
            {isTailoring && tailoringStage && <span className={styles.stageText}>{tailoringStage}</span>}
          </div>
          ...
          {tailoredBullets && (
            <div className={styles.results}>
                ...
                <div className={styles.resultsActions}>
                  {getOverallScore() !== null && (
                    <div className={styles.overallScore}>
                      ...
                      <div className={styles.scoreBadge}>{getOverallScore()}%</div>
                    </div>
                  )}
                  <Button variant="secondary" size="sm" onClick={copyAllTailored} disabled={!tailoredBullets.some(b => b.status === 'accepted')}>
                  ...
                  <Button variant="outline" size="sm" onClick={exportTailored} disabled={!tailoredBullets.some(b => b.status === 'accepted')}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
                      <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
                      <polyline points="17 8 12 3 7 8" />
                      <line x1="12" y1="3" x2="12" y2="15" />
              ...
              {tailoredBullets.map((bullet, i) => (
                <div key={i} className={`${styles.bulletResult} ${bullet.status === 'accepted' ? styles.accepted : ''} ${bullet.status === 'rejected' ? styles.rejected : ''}`}>
                  <div className={styles.bulletPair}>
                    <div className={styles.bulletOriginal}>
                      <span className={styles.bulletLabel}>Original:</span>
                      <p>{bullet.original}</p>
                    </div>
                    <div className={styles.bulletTailored}>
                      <div className={styles.bulletTailoredHeader}>
                        <span className={styles.bulletLabel}>Tailored:</span>
                        <span className={`${styles.statusBadge} ${styles[bullet.status || 'pending']}`}>
                          {bullet.status === 'accepted' ? '✓ Accepted' : bullet.status === 'rejected' ? '✗ Rejected' : '⏳ Pending'}
                        </span>
                      </div>
                      <p className={styles.tailoredText}>{bullet.tailored}</p>
                      {(bullet.keywords_added && bullet.keywords_added.length > 0) && (
                        <div className={styles.keywordsAdded}>
                          <span className={styles.keywordsLabel}>Keywords added:</span>
                          {bullet.keywords_added.map((kw, idx) => (
                            <span key={idx} className={styles.keywordAdded}>{kw}</span>
                          ))}
                        </div>
                      )}
                      <div className={styles.bulletActions}>
                        {bullet.status !== 'accepted' && (
                          <Button variant="secondary" size="sm" onClick={() => acceptBullet(i)}>
                        ...
                        {bullet.status === 'accepted' && (
                          <Button variant="secondary" size="sm" onClick={() => rejectBullet(i)}>
                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
                        ...
                        <Button variant="outline" size="sm" onClick={() => regenerateBullet(i)} disabled={isTailoring}>
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
